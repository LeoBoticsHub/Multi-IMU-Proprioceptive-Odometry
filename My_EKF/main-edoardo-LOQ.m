...
param.has_mocap = 1;
param.mipo_use_foot_ang_contact_model = 0; % 0 means use zero velocity model
...
param.proc_n_foot1_bg = 1e-4;  
param.proc_n_foot2_bg = 1e-4;  
param.proc_n_foot3_bg = 1e-4;  
param.proc_n_foot4_bg = 1e-4; 
...
figure(100)
plot_start=1;
plot_end = size(state(end,:),2);
hold on;
grid on;
plot3(resampled_data.pos_mocap.Data(:,1),resampled_data.pos_mocap.Data(:,2),resampled_data.pos_mocap.Data(:,3), 'LineWidth',1.3)
plot3(state(1,plot_start:plot_end),state(2,plot_start:plot_end),state(3,plot_start:plot_end), 'LineWidth',1.3);
title('Robot position through time in x and y coordinates ')
legend("Ground truth", "MIPO", "Location","northeast")
axis equal
xlabel("X Position (m)")
ylabel("Y Position (m)")
view(-0,90)
...
grid on;
%plot3(resampled_data.pos_mocap.Data(:,1),resampled_data.pos_mocap.Data(:,2),resampled_data.pos_mocap.Data(:,3), 'LineWidth',1.3)
plot(resampled_data.j_acc.Time(200:end,1), state(13,plot_start:plot_end)', 'LineWidth',1.3);
plot(resampled_data.j_acc.Time(200:end,1), resampled_data.om_b_IMU.Data(200:end,1)', 'LineWidth',1);
title('Robot position through time in x and y coordinates ')
legend("State", "IMU")
axis equal
xlabel("X Position (m)")
