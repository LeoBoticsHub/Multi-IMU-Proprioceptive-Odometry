function [om_b_b, a_b_b, om_dot_b_b, om_b_b_avg, a_b_b_avg] = my_equations(resampled_data, param)
steps = size(resampled_data.acc_b_IMU.Time,1);
init_idx = 1; % choose the best starting index based on dataset
end_idx =  steps;
N = end_idx - init_idx + 1; 
dt = resampled_data.acc_b_IMU.Time(2)- resampled_data.acc_b_IMU.Time(1);
...
for idx=init_idx:end_idx % current step index
    ...
    for leg_id=1:4
        ...
        R_fb = R_fb_func(l_phik);     % rotation matrix from foot to body
        omega_f_b_IMUs = R_fb*param.R_fs{leg_id}*omega_f_f_IMUs(idx_range); 
...
for idx=init_idx:end_idx 
    ...
    for leg_id=1:4
        ...
        R_fb = R_fb_func(l_phik);     % rotation matrix from foot to body
        acc_f_b_IMU = R_fb*param.R_fs{leg_id}*(acc_f_f_IMUs(idx_range));
