...
% robot kinematic and foot IMU sensors cosndiering only the legs in
% contact.
...
contact_mode_data = resampled_data.foot_contact.Data(init_idx:end_idx, 1:4);
...
% Average calculation considering only legs in contact
for k = 1:N
    legs_in_contact = find(contact_mode_data(k, :) >= 0.5);
    for i=1:3
        if ~isempty(legs_in_contact)
            om_b_b_avg(i, k) = mean(om_b_b(i, k, legs_in_contact), 3, 'omitnan');
            a_b_b_avg(i, k) = mean(a_b_b(i, k, legs_in_contact), 3, 'omitnan');
        end
    end
end
