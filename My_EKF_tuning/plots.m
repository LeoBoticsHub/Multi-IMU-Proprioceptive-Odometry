...
% figureName = ['n_acc_formula = ', num2str(i)];%,' n_om_formula_y = ', num2str(i)];
...
figure();
title('Body Linear Velocity from EKF vs Ground Truth')
for i = 1:3
    subplot(3, 1, i);
    grid on;
    hold on;
    plot(resampled_data.om_b_IMU.Time(param.data_start_idx:param.data_end_idx), mocap_vel(param.data_start_idx:param.data_end_idx,i), 'LineWidth',1);
    plot(resampled_data.om_b_IMU.Time(param.data_start_idx:param.data_end_idx), state(3+i, 1:end-1)', 'LineWidth',1.3);
    xlabel('Time (s)');
    ylabel([velocity(i), ' (m/s)']);
    legend("Ground Truth", "EKF", "Location","north")
    title(['Velocity comparizon - Axis ', num2str(i)]);
end